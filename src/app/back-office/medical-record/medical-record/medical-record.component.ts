...
export class MedicalRecordComponent implements OnInit{
  ...
  addConstants(medicalRecordId: number) {
    const modalRef = this.modalService.create({
      ...
      nzFooter: null,
      // Vous pouvez passer des paramètres en utilisant la propriété 'data'
    ...
    // Vous pouvez alors accéder aux propriétés et méthodes du composant
    modalComponent.recordId = medicalRecordId;
    modalComponent.showModal();
  }

  addAnalisis(medicalRecordId: number){
    this.addAnalyses = true;
  ...
  onSubmit(){
    if(this.recordForm.valid){
      ...
      }else if(this.addAnalyses){

      }else if(this.addConstant){
